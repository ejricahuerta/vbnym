...
export const getUpcomingGamesWithSignups = cache(
  async (): Promise<{
    games: Game[];
...
export const getGameWithSignups = cache(
  async (id: string): Promise<{ game: Game; signups: Signup[] } | null> => {
    const hasSupabase =
      process.env.NEXT_PUBLIC_SUPABASE_URL &&
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
...
/** All games for admin list (any listing state), ordered by date ascending. */
export const listGamesForAdmin = cache(
  async (): Promise<{ games: Game[]; error: string | null }> => {
    let games: Game[] = [];
    ...
    try {
      ...
      else games = ((data ?? []) as Game[]).map(normalizeGame);
    } catch (e) {
