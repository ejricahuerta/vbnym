...
const LINK_KIND = "link" as const;
const SESSION_KIND = "session" as const;
...
export function createPlayerMagicLinkToken(email: string): string | null {
  ...
  const body: LinkPayload = {
    ...
    k: LINK_KIND,
    email: normalized,
    exp: Date.now() + PLAYER_MAGIC_LINK_TTL_MS,
  };
  return encodeToken(body, secret);
}

export function verifyPlayerMagicLinkToken(
  token: string
): { email: string } | null {
  const secret = getSecret();
  if (!secret) return null;
...
export function createPlayerRecoverSessionToken(email: string): string | null {
  const secret = getSecret();
  if (!secret) return null;
  const normalized = email.trim().toLowerCase();
  const body: SessionPayload = {
    ...
    k: SESSION_KIND,
    email: normalized,
    exp: Date.now() + PLAYER_RECOVER_SESSION_MAX_AGE_SEC * 1000,
  };
  return encodeToken(body, secret);
}

export function verifyPlayerRecoverSessionToken(
